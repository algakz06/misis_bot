...
async def get_first_name(message: Message):
    if not re.match("^([А-ЯЁ][а-яё]*)$", message.text):
        await message.answer('Неверный формат, напишите свое имя, например "Даниил"')
        return
...
async def get_last_name(message: Message):
    if not re.match("^([А-ЯЁ][а-яё]*)$", message.text):
        ...
        return
    await message.answer(
        "Отлично, двигаемся дальше\n\nОтправьте свой город (например, Москва)"
    )
...
async def get_email(message: Message):
    if not requests.get(
        ...
        return
    await message.answer(
        "Последний шаг — твой номер телефона!\
        \nОтправь его в формате 89999999999"
    )
